...
def test_J1():
    ...
    spin1 = jnp.array([[[1,0] for _ in range(model.L2)] for _ in range(model.L1)])
    spin2 = jnp.array([[[1,0] for _ in range(model.L2)] for _ in range(model.L1)])
    ...
    spinud = spin1.at[1, 0].set([0, 1])
    spindu = spin1.at[0, 0].set([0, 1])
...
def test_SzSz():
    ...
    print(rbm.expectation_value(SzSz_(0,0,0,1,model), batch[0]))
    ...
    assert abs(jnp.mean(average_expectations) - (0.25 * weight**2 - 0.25)/(weight**2 + 1)) < 4*np.std(average_expectations)/np.sqrt(400), f"Mixed state has wrong expectation: {np.mean(average_expectations)}"
    # rbm = RBM(model)
...
def test_expectation():
    ...
    def calculate_Sz_expectation_brute_force(spins):
        return jnp.mean(jnp.sum(batch[:, :, :, 0]/2 - batch[:, :, :, 1]/2, axis=(1, 2)))
